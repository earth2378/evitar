...
def deployContract(w3):
    ...
    for row in csv_reader:
        ...
        tx = txF.createContract(w3, nonce, int(
            row[3]), 10000000, 10000000, row[6], private_key)
...
def getAvailableContract(w3):
    ...
    csv_writer = csv.writer(csv_write, delimiter=',')
    csv_writer.writerow(
        ["txHash", "oldCreator", "oldContractAddress", "newContractAddress", "status"])
    for i in range(0, w3.eth.blockNumber+1):
        txList = w3.eth.getBlock(i).transactions
        for tx in txList:
            data = next(csv_reader, None)
            txReceipt, txResult = w3.eth.getTransactionReceipt(
            ...
            csv_writer.writerow([tx.hex(), data[1], data[2], w3.toChecksumAddress(
                txReceipt.contractAddress), msg])
