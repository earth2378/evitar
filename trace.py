...
def createTree(trace_list):
    ...
    for trace in trace_list:
        ...
        if(trace[1] == None or trace[1] == ""):
            ...
            try:
                tree.create_node(isNone(trace[2]), tuple(
                    trace[1].split(',')), tuple(trace[1].split(',')[:-1]))
            except:
                # print(trace)
                pass
...
def groupTraceByHash(dstFile, csv_reader):
    ...
    for row in csv_reader:
        ...
        # row = list(row.values())
        filteredRow = [row[0], row[2], row[3]]
        if(filteredRow[0] == txTmp):
            trace_list.append(filteredRow)
        else:
            if(txTmp != ''):
                traceTree = createTree(trace_list)
                processError(csv_writer, traceTree, txTmp)
            txTmp = filteredRow[0]
            trace_list = [filteredRow]
...
def getTransactionsError(dst):
    # read trace file
    # file = './source/traces_trim_evitar_{}_new.csv'.format(dst)
    file = './source/traces_trim_maxgas.csv'
    csv_read = open(file, "r")
    csv_reader = csv.reader(csv_read, delimiter=",")
    csv_reader.__next__()

    # dstFile = './result/error_identify_evitar_{}_new.csv'.format(dst)
    dstFile = './result/error_identify_maxgas.csv'
